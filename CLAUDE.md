...
- **Known issue — `AdvancedMarkerElement` positioning drift**: `google.maps.marker.AdvancedMarkerElement` (used for custom map markers, shape note-labels, live team member pins, and intel pins) has a confirmed, reproducible positioning-drift bug — its on-screen position visibly separates from its true lat/lng the further the map is zoomed from wherever it was last drawn, correcting back to accurate near that original zoom level. Confirmed present under both a real vector Map ID and a real raster Map ID (see `components/Map.tsx`'s `MAP_ID_VECTOR`/`MAP_ID_RASTER`), so it is not a rendering-mode/Map-ID issue. Not fixable from outside the class: forcing a fresh `.position` assignment, or even a full detach/reattach (`.map = null` then `.map = map`) on the map's `idle` event, doesn't stop it — `AdvancedMarkerElement` recalculates its position continuously as part of the map's own render loop during a gesture, not once at the end, so any one-shot correction gets overwritten by the very next frame. The planned fix is migrating these overlays to `google.maps.OverlayView`, which computes its own pixel position from the map's projection on every redraw rather than trusting Google's internal state. When that migration happens, do it as its own dedicated, incremental change (one overlay type at a time, each independently revertible) and document it in this section — including that `AdvancedMarkerElement`'s current implementation stays intact in git history, so reverting to it later (e.g. if a future Google Maps JS API release fixes this bug) is a reasonable option to revisit, not a rewrite from scratch.
