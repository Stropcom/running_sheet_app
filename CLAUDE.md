...
- **Technically enforced, not just documented**: `.claude/settings.json` denies WebFetch/curl/wget to known AI API hosts and wires up `.claude/hooks/check-no-runtime-ai.sh`, which blocks any Write/Edit introducing a banned AI SDK/host pattern outside `dev-tools/`, `scripts/dev/`, or a `_core` directory. `scripts/check-no-runtime-ai.sh` re-runs the same scan as the authoritative gate in CI (`.github/workflows/no-runtime-ai.yml`, on every push/PR). The banned-pattern list currently only matches literal SDK/package/host strings (e.g. `openai`, `anthropic`) — it does **not** catch someone importing the existing dead `_core/llm.ts` helpers by name (e.g. `invokeLLM(`) into a live route, since `_core` is deliberately excluded from the scan. If you extend this list, keep it in sync across both scripts.
