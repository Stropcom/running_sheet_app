...
function safeFormat(d: Date | string | null | undefined, fmt: string, fallback = ""): string {
  ...
  static activators = [
    {
      ...
      handler: ({ nativeEvent }: { nativeEvent: PointerEvent }) => shouldHandleEvent(nativeEvent.target),
    },
  ];
}
...
type ShiftCode = "" | "d" | "a" | "r" | "o" | "l" | "c" | "tt" | "dep" | "doc" | "adoc" | "ad" | "aoc";
...
const ALL_DATES = eachDayOfInterval({ start: ROSTER_START_DATE, end: ROSTER_END_DATE });
...
function ShiftEditSheet({
  open, memberName, date, currentCode, currentComment, currentIsActing, currentShiftTime, isAdmin,
  onClose, onSaveShift, onSaveAnnotation, onCopyShift,
}: {
  open: boolean; memberName: string; date: string; currentCode: string;
  currentComment: string; currentIsActing: boolean; currentShiftTime?: string | null; isAdmin: boolean;
  onClose: () => void;
  ...
  const defaultTime = (code: string) => SHIFT_TIMES[code as keyof typeof SHIFT_TIMES] ?? "";
  ...
  const label = SHIFT_LABELS[currentCode as keyof typeof SHIFT_LABELS] ?? currentCode;
  const time = currentShiftTime ?? (SHIFT_TIMES[currentCode as keyof typeof SHIFT_TIMES] ?? "");

  const inner = (
    <div>
      {currentCode && (
        <div className="flex items-center gap-2 mb-4">
          <div className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium", shiftClass(currentCode))}>
      ...
      {isAdmin && (
        <div className="flex items-center justify-between rounded-xl border border-border p-3.5 mb-4 bg-muted/30">
          <div className="flex items-center gap-2.5">
            <Star className={cn("h-4 w-4", isActing ? "text-amber-500 fill-amber-400" : "text-muted-foreground")} />
            <div>
              <p className="text-sm font-medium">Acting</p>
              <p className="text-xs text-muted-foreground">Applies to existing shift — no shift change needed</p>
            </div>
          </div>
          <Switch checked={isActing} onCheckedChange={(v) => { setIsActing(v); onSaveAnnotation(comment.trim() || null, v); }} />
        </div>
      )}
      ...
      {isAdmin && (
        <div className="mb-5">
          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2 block">Shift Note</Label>
          <Textarea placeholder="Add a note for this shift…" value={comment} onChange={e => setComment(e.target.value)} className="resize-none h-20 text-sm" />
          <Button className="w-full mt-2" size="sm" onClick={() => { onSaveAnnotation(comment.trim() || null, isActing); onClose(); }}>
            Save Note
          </Button>
        </div>
      )}
      ...
      {isAdmin && (
        <div className="mb-4">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Change Shift</p>
          <div className="grid grid-cols-4 gap-1.5">
            {SHIFT_CODES.map(c => {
              const t = SHIFT_TIMES[c as keyof typeof SHIFT_TIMES];
              return (
                <button key={c} onClick={() => {
                  const newTime = SHIFT_TIMES[c as keyof typeof SHIFT_TIMES] ?? "";
                  setPendingCode(c);
                  ...
                  className={cn(
                    ...
                    shiftClass(c),
                    pendingCode === c ? "border-white/60 shadow-md scale-[1.03]" : "border-transparent",
                  )}>
                  <span className="absolute inset-x-1 top-1.5 text-[11px] font-bold uppercase text-center block">{c}</span>
          ...
          {pendingCode && SHIFT_TIMES[pendingCode as keyof typeof SHIFT_TIMES] !== undefined && (
            <div className="mt-3 flex items-center gap-2">
              <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">Start Time</Label>
              <input
                type="time"
                value={shiftTime}
          ...
          <div className="flex gap-2 mt-3">
            {pendingCode && (
              <Button className="flex-1" size="sm" onClick={() => { onSaveShift(pendingCode, shiftTime || null); onClose(); }}>
                Apply
              </Button>
            )}
            {currentCode && (
              <button onClick={() => { onSaveShift("", null); onClose(); }}
        ...
        <SheetContent side="bottom" className="rounded-t-2xl max-h-[92vh] overflow-y-auto pb-8">
          <SheetHeader className="mb-4">
            <SheetTitle className="flex items-center gap-2 text-base">
              ...
              <span className="text-muted-foreground text-sm">{safeFormat(date, "EEE d MMM yyyy")}</span>
            </SheetTitle>
          </SheetHeader>
          {inner}
        </SheetContent>
...
function BulkApplySheet({
  ...
  onClose: () => void;
  onApply: (code: ShiftCode | null, isActing: boolean, actingOnly: boolean, shiftTime: string | null) => void;
}) {
  const isMobile = useIsMobile();
    ...
    <div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {actingOnly ? "Apply acting to existing shifts only (no shift change)" : "Also apply acting indicator"}
            </p>
          </div>
        </div>
        <Switch checked={isActing} onCheckedChange={(v) => { setIsActing(v); if (v) {} else setActingOnly(false); }} />
      </div>
      ...
      {isActing && (
        <div className="flex items-center justify-between rounded-xl border border-amber-200 p-3 mb-4 bg-amber-50">
          <div>
            <p className="text-sm font-medium text-amber-800">Acting only (no shift change)</p>
            <p className="text-xs text-amber-600">Toggle acting on selected cells without changing their shift code</p>
          </div>
      ...
      {!actingOnly && (
        <>
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Select Shift</p>
          <div className="grid grid-cols-4 gap-1.5 mb-4">
            {SHIFT_CODES.map(c => {
              const t = SHIFT_TIMES[c as keyof typeof SHIFT_TIMES];
              return (
                <button key={c} onClick={() => { setSelectedCode(c as ShiftCode); setShiftTime(defaultTime(c)); }}
                  className={cn(
                    "relative h-14 rounded-lg text-[10px] font-medium transition-all active:scale-95 overflow-hidden border-2",
                    shiftClass(c),
                    selectedCode === c ? "border-white/60 shadow-md scale-[1.03]" : "border-transparent",
                  )}>
                  <span className="absolute inset-x-1 top-1.5 text-[11px] font-bold uppercase text-center block">{c}</span>
                  <span className="absolute inset-x-1 bottom-1 text-[7px] opacity-60 leading-tight truncate text-center block">
                    {SHIFT_LABELS[c]}{t ? ` · ${t}` : ""}
          ...
          {/* Time picker */}
          {SHIFT_TIMES[selectedCode as keyof typeof SHIFT_TIMES] !== undefined && (
            <div className="flex items-center gap-2 mb-4">
              <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">Start Time</Label>
              <input
                type="time"
      ...
      <div className="flex gap-2">
        ...
        <Button className="flex-1" onClick={() => onApply(actingOnly ? null : selectedCode, isActing, actingOnly, actingOnly ? null : (shiftTime || null))}>
          {actingOnly ? `Set Acting on ${count} Cell${count !== 1 ? "s" : ""}` : `Apply to ${count} Cell${count !== 1 ? "s" : ""}`}
        </Button>
      </div>
    </div>
  );
  ...
  if (isMobile) {
    return (
      <Sheet open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
        <SheetContent side="bottom" className="rounded-t-2xl max-h-[92vh] overflow-y-auto pb-8">
          <SheetHeader className="mb-4">
            <SheetTitle>Apply Shift to {count} Cell{count !== 1 ? "s" : ""}</SheetTitle>
          </SheetHeader>
          {inner}
        </SheetContent>
      </Sheet>
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-sm w-full max-h-[90vh] overflow-y-auto">
        <DialogHeader className="mb-2">
          <DialogTitle>Apply Shift to {count} Cell{count !== 1 ? "s" : ""}</DialogTitle>
        </DialogHeader>
        {inner}
      </DialogContent>
    </Dialog>
...
function BulkCopySheet({
  open, members, onClose, onCopy, isLoading,
}: {
  open: boolean;
  members: Member[];
  onClose: () => void;
  onCopy: (sourceMemberId: number, targetMemberIds: number[], startDate: string, endDate: string) => void;
  isLoading: boolean;
}) {
  const isMobile = useIsMobile();
  ...
  const canSubmit = sourceMemberId !== "" && targetMemberIds.size > 0 && startDate <= endDate;

  const inner = (
    <div className="space-y-4">
      {/* Source member */}
      <div>
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">Copy From</p>
        <select
          value={sourceMemberId}
          onChange={e => setSourceMemberId(e.target.value === "" ? "" : Number(e.target.value))}
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">From Date</p>
          <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
            min={ROSTER_START} max={ROSTER_END}
            className="w-full h-9 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
        </div>
        <div>
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">To Date</p>
      ...
      <div>
        ...
        <div className="max-h-48 overflow-y-auto rounded-md border border-border divide-y divide-border">
          {members
            ...
            .map(m => (
              <label key={m.id} className="flex items-center gap-2.5 px-3 py-2 cursor-pointer hover:bg-muted/50 transition-colors">
                <input
                  type="checkbox"
      ...
      <div className="flex gap-2 pt-1">
        ...
        <Button
          ...
          onClick={() => {
            ...
            onCopy(sourceMemberId, Array.from(targetMemberIds), startDate, endDate);
          }}
        >
          {isLoading ? "Copying…" : `Copy to ${targetMemberIds.size} Member${targetMemberIds.size !== 1 ? "s" : ""}`}
        </Button>
      </div>
    </div>
  );
...
export default function RosterPage() {
  ...
  const dragScrollRef = useRef<{ startX: number; startScrollLeft: number; active: boolean }>({
  ...
  const handleGridPointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (target.closest('[data-no-dnd]') || target.closest('button') || target.closest('[role="button"]')) return;
    if (!scrollRef.current) return;
    dragScrollRef.current = { startX: e.clientX, startScrollLeft: scrollRef.current.scrollLeft, active: true };
    try { (e.currentTarget as HTMLDivElement).setPointerCapture(e.pointerId); } catch { /* iOS Safari may not support setPointerCapture on all elements */ }
  ...
  const visibleMonthIndex = useMemo(() => {
    ...
    const visibleDate = ALL_DATES[Math.min(Math.max(0, visibleDayIdx), ALL_DATES.length - 1)];
  ...
  const goToPrevMonth = () => {
    ...
    scrollToDate(format(d < ROSTER_START_DATE ? ROSTER_START_DATE : d, "yyyy-MM-dd"));
  };
  ...
  const goToNextMonth = () => {
    ...
    const mIdx = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].indexOf(mon);
    const d = new Date(parseInt(yr), mIdx, 1);
    scrollToDate(format(d > ROSTER_END_DATE ? ROSTER_END_DATE : d, "yyyy-MM-dd"));
  };

  // Bulk edit
  const [bulkMode, setBulkMode] = useState(false);
  const [selectedCells, setSelectedCells] = useState<Map<string, { memberId: number; date: string }>>(new Map());
  ...
  const [copiedBlock, setCopiedBlock] = useState<{
    cells: { memberId: number; date: string; shiftCode: string; comment: string; isActing: boolean }[];
  ...
  const [editSheet, setEditSheet] = useState<{
    open: boolean; memberId: number; memberName: string; date: string;
    currentCode: string; currentComment: string; currentIsActing: boolean;
    currentShiftTime: string | null;
  } | null>(null);

  // Copy/paste clipboard
  const [clipboard, setClipboard] = useState<{ code: string; comment: string; isActing: boolean } | null>(null);
  ...
  const { data: membersData, refetch: refetchMembers } = trpc.ctoRoster.members.list.useQuery();
  const { data: shiftsData, isLoading } = trpc.ctoRoster.roster.getRange.useQuery(
  ...
  const updateShift = trpc.ctoRoster.roster.updateShift.useMutation({
    onMutate: async (vars) => {
      ...
      const prev = utils.ctoRoster.roster.getRange.getData({ startDate: ROSTER_START, endDate: ROSTER_END });
      utils.ctoRoster.roster.getRange.setData(
        { startDate: ROSTER_START, endDate: ROSTER_END },
        (old) => {
          if (!old) return old;
          const updated = old.map((s) => {
            if (s.memberId === vars.memberId && s.shiftDate === vars.shiftDate) {
              return {
                ...s,
                shiftCode: vars.shiftCode,
                shiftTime: vars.shiftTime !== undefined ? (vars.shiftTime ?? null) : s.shiftTime,
                comment: vars.comment !== undefined ? vars.comment : s.comment,
                isActing: vars.isActing !== undefined ? vars.isActing : s.isActing,
              };
            }
            return s;
          });
    ...
    onError: (e, _vars, ctx) => {
      // Roll back on error
      if (ctx?.prev) utils.ctoRoster.roster.getRange.setData({ startDate: ROSTER_START, endDate: ROSTER_END }, ctx.prev);
  ...
  const reorderMembers = trpc.ctoRoster.members.reorder.useMutation({
    onSuccess: () => { refetchMembers(); setLocalMembers(null); },
    onError: (e) => { toast.error(`Failed to move: ${e.message}`); setLocalMembers(null); },
  });
  ...
  const visualRowMembers: Member[] = useMemo(() => {
    const teams = (teamsData as Team[] ?? []).slice().sort((a, b) => a.sortOrder - b.sortOrder);
    return teams.flatMap(t => displayMembers.filter(m => m.teamId === t.id).sort((a, b) => a.sortOrder - b.sortOrder));
  ...
  const handlePasteBlock = useCallback((targetMemberId: number, targetDate: string) => {
    if (!copiedBlock) return;
    const { cells, memberIds, dates } = copiedBlock;
    // Use visualRowMembers (team-grouped order) so row deltas match the grid
    const anchorMemberIdx = visualRowMembers.findIndex(m => m.id === memberIds[0]);
    const anchorDateIdx = ALL_DATES.findIndex(d => format(d, "yyyy-MM-dd") === dates[0]);
    const targetMemberIdx = visualRowMembers.findIndex(m => m.id === targetMemberId);
    const targetDateIdx = ALL_DATES.findIndex(d => format(d, "yyyy-MM-dd") === targetDate);
    const rowDelta = targetMemberIdx - anchorMemberIdx;
    const colDelta = targetDateIdx - anchorDateIdx;
    let pasted = 0;
    for (const cell of cells) {
      const srcMemberIdx = visualRowMembers.findIndex(m => m.id === cell.memberId);
      const srcDateIdx = ALL_DATES.findIndex(d => format(d, "yyyy-MM-dd") === cell.date);
      const dstMemberIdx = srcMemberIdx + rowDelta;
      const dstDateIdx = srcDateIdx + colDelta;
      if (dstMemberIdx < 0 || dstMemberIdx >= visualRowMembers.length) continue;
  ...
  const handleCellTap = useCallback((memberId: number, memberName: string, date: string, shift: ShiftData | undefined) => {
    ...
    if (bulkCopyMode) {
      ...
      setSelectedCells(prev => {
        ...
        if (next.has(key)) next.delete(key); else next.set(key, { memberId, date });
        return next;
      });
      return;
    }
    ...
    setEditSheet({ open: true, memberId, memberName, date, currentCode: shift?.shiftCode ?? "", currentComment: shift?.comment ?? "", currentIsActing: shift?.isActing ?? false, currentShiftTime: shift?.shiftTime ?? null });
  }, [bulkMode, bulkCopyMode, copiedBlock, handlePasteBlock]);

  const handleCellPointerDown = useCallback((memberId: number, date: string) => {
  ...
  const handleCellPointerEnter = useCallback((memberId: number, date: string) => {
  ...
  const handlePointerUp = useCallback(() => { isDraggingSelection.current = false; }, []);
  useEffect(() => {
    window.addEventListener("pointerup", handlePointerUp);
    return () => window.removeEventListener("pointerup", handlePointerUp);
  }, [handlePointerUp]);

  const handleSaveShift = useCallback((memberId: number, date: string, code: string) => {
    updateShift.mutate({ memberId, shiftDate: date, shiftCode: code as ShiftCode });
  }, [updateShift]);

  const handleSaveAnnotation = useCallback((memberId: number, date: string, currentCode: string, comment: string | null, isActing: boolean) => {
    updateShift.mutate({ memberId, shiftDate: date, shiftCode: currentCode as ShiftCode, comment, isActing });
  }, [updateShift]);

  const handleCopyCell = useCallback((shift: ShiftData | undefined) => {
    if (!shift?.shiftCode) { toast.error("Nothing to copy — cell is empty"); return; }
    setClipboard({ code: shift.shiftCode, comment: shift.comment ?? "", isActing: shift.isActing ?? false });
    toast.success(`Copied: ${SHIFT_LABELS[shift.shiftCode as keyof typeof SHIFT_LABELS] ?? shift.shiftCode}`);
  }, []);

  const handlePasteCell = useCallback((memberId: number, date: string) => {
    if (!clipboard) { toast.error("Nothing in clipboard — copy a shift first"); return; }
    updateShift.mutate({ memberId, shiftDate: date, shiftCode: clipboard.code as ShiftCode, comment: clipboard.comment || null, isActing: clipboard.isActing });
    toast.success(`Pasted: ${SHIFT_LABELS[clipboard.code as keyof typeof SHIFT_LABELS] ?? clipboard.code}`);
  }, [clipboard, updateShift]);

  const handleBulkApply = useCallback((code: ShiftCode | null, isActing: boolean, actingOnly: boolean, shiftTime: string | null) => {
    ...
    if (actingOnly) {
      ...
      for (const { memberId, date } of Array.from(selectedCells.values())) {
        const currentCode = shiftMap.get(memberId)?.get(date)?.shiftCode ?? "";
        updateShift.mutate({ memberId, shiftDate: date, shiftCode: currentCode as ShiftCode, isActing });
      }
      setSelectedCells(new Map());
      setBulkSheetOpen(false);
      setBulkMode(false);
    ...
    for (const [memberId, dates] of Array.from(byMember.entries())) {
      bulkUpdate.mutate({ memberId, dates, shiftCode: code as ShiftCode, isActing, shiftTime });
    }
  ...
  const handleCopyBlock = useCallback(() => {
    ...
    const memberIds = Array.from(new Set(cells.map(c => c.memberId))).sort((a, b) => {
    ...
    setSelectedCells(new Map());
    toast.success(`Block copied (${cells.length} cell${cells.length !== 1 ? "s" : ""}) — click a cell to paste`);
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (!localMembers && membersData) setLocalMembers([...(membersData as Member[])]);
  };
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (overId.startsWith("team-zone-")) setOverTeamId(parseInt(overId.replace("team-zone-", "")));
    else {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setActiveDragId(null); setOverTeamId(null);
    ...
    for (const [, group] of Array.from(teamGroups.entries())) {
      const withoutActive = group.filter((m: Member) => m.id !== activeId).sort((a: Member, b: Member) => a.sortOrder - b.sortOrder);
      let insertIdx = withoutActive.length;
      if (insertBeforeMemberId !== null && group.some((m: Member) => m.id === activeId)) {
        const beforeIdx = withoutActive.findIndex((m: Member) => m.id === insertBeforeMemberId);
        if (beforeIdx !== -1) insertIdx = beforeIdx;
      }
      withoutActive.splice(insertIdx, 0, activeMember);
      withoutActive.forEach((m: Member, i: number) => { m.sortOrder = i; });
    }
    setLocalMembers(updated);
    reorderMembers.mutate(updated.map(m => ({ id: m.id, teamId: m.teamId, sortOrder: m.sortOrder })));
  };

  const activeMember = activeDragId ? displayMembers.find(m => m.id === activeDragId) : null;
  ...
  return (
    <DashboardLayout>
    <div className="flex flex-col select-none px-4 py-3" style={{ height: "calc(100vh - 0px)", minHeight: 0, overflow: 'hidden' }}>

            {/* ── Toolbar ───────────────────────────────────────────────────────── */}
      <div className="flex flex-col gap-1.5 mb-3 flex-shrink-0">
        {/* Row 1: title + Today button (Today visible on mobile only here) */}
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-semibold tracking-tight mr-auto">Team Duty Roster</h1>
          <Button variant="outline" size="sm" className="gap-1.5 h-8 text-xs sm:hidden" onClick={() => scrollToDate(today)}>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none">
          ...
          <div className="flex items-center gap-1">
            <Button variant="outline" size="icon" className="h-8 w-8" onClick={goToPrevMonth} disabled={visibleMonthIndex === 0}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <span className="text-sm font-medium min-w-[76px] text-center tabular-nums">{MONTHS[visibleMonthIndex]}</span>
            <Button variant="outline" size="icon" className="h-8 w-8" onClick={goToNextMonth} disabled={visibleMonthIndex === MONTHS.length - 1}>
              <ChevronRight className="h-4 w-4" />
            </Button>
          </div>
          {/* Today button — tablet/desktop only, between month nav and Bulk Edit */}
          <Button variant="outline" size="sm" className="gap-1.5 h-8 text-xs hidden sm:flex flex-shrink-0" onClick={() => scrollToDate(today)}>
            <CalendarDays className="h-3.5 w-3.5" />
            Today
          </Button>
          {/* Bulk Edit */}
          {isAdmin && (
            <button
              onClick={() => { setBulkMode(v => !v); setSelectedCells(new Map()); }}
              className={cn(
                "flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-colors h-8",
                bulkMode ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted"
              )}
            >
          ...
          {isAdmin && (
            <button
              ...
              className={cn(
                "flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-colors h-8",
                bulkCopyMode ? "bg-blue-600 text-white border-blue-600" : "border-border text-muted-foreground hover:bg-muted"
              )}
            >
          ...
          {bulkCopyMode && selectedCells.size > 0 && (
            <Button size="sm" className="h-8 text-xs bg-blue-600 hover:bg-blue-700" onClick={handleCopyBlock}>
          ...
          {isAdmin && copiedBlock && (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-blue-50 border border-blue-200 text-blue-700 text-xs font-medium h-8">
              ...
              <span className="hidden sm:inline">Block ready ({copiedBlock.cells.length}) — tap paste start</span>
              <span className="sm:hidden">{copiedBlock.cells.length} ready</span>
              <button onClick={() => setCopiedBlock(null)} className="ml-1 text-blue-400 hover:text-blue-700">×</button>
            </div>
          )}
          {/* Apply shift button */}
          {bulkMode && selectedCells.size > 0 && (
            <Button size="sm" className="h-8 text-xs" onClick={() => setBulkSheetOpen(true)}>
          ...
          {isAdmin && clipboard && (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-amber-50 border border-amber-200 text-amber-700 text-xs font-medium h-8">
              <ClipboardPaste className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">Clipboard: {SHIFT_LABELS[clipboard.code as keyof typeof SHIFT_LABELS] ?? clipboard.code}</span>
              <span className="sm:hidden">{SHIFT_LABELS[clipboard.code as keyof typeof SHIFT_LABELS] ?? clipboard.code}</span>
              <button onClick={() => setClipboard(null)} className="ml-1 text-amber-400 hover:text-amber-700">×</button>
            </div>
          )}
        </div>
      </div>
      ...
      */}
      ...
      >
        {isLoading ? (
          ...
          <div style={{ minWidth: `${totalContentWidth}px` }} className="relative pb-2">

            {/* ── Frozen date header row ─────────────────────────────────── */}
            <div
              className="flex gap-[3px] mb-[3px]"
              style={{ position: "sticky", top: 0, zIndex: 30, background: "var(--color-background)", paddingBottom: "2px" }}
            >
              <div
                ...
                style={{ width: `${nameColWidthForGrid}px`, height: `${HEADER_HEIGHT}px`, position: "sticky", left: 0, zIndex: 40, background: "var(--color-card)" }}
              >
                Member
              </div>
              {ALL_DATES.map(d => {
                  ...
                  <div key={ds}
                    className={cn(
                      ...
                      isToday ? "bg-primary text-primary-foreground shadow-sm"
                        : isWeekend ? "bg-muted/60 text-muted-foreground"
                        : "bg-card text-foreground border border-border/50 shadow-sm",
                    )}
                    style={{ width: `${COL_WIDTH}px`, height: `${HEADER_HEIGHT}px` }}
                  >
                    {isMonthStart && (
                    ...
                    <span className="text-[9px] uppercase tracking-wide opacity-60 leading-none mt-1">{format(d, "EEE")}</span>
                    <span className="text-xs font-bold leading-none mt-0.5">{format(d, "d")}</span>
                  </div>
                );
              })}
            </div>
            ...
            {(teamsData as Team[] ?? []).map(team => {
              const teamMembers = displayMembers.filter(m => m.teamId === team.id);
              const isSurveillance = team.name === "Team 1" || team.name === "Team 2";
              return (
                <div key={team.id} className="mb-1">
                  ...
                  <div className="flex gap-[3px] mb-[3px]">
                    <div
                      ...
                      style={{ width: `${nameColWidthForGrid}px`, position: "sticky", left: 0, zIndex: 20, background: "var(--color-sidebar)" }}
                    >
                      {team.name}
                    </div>
                    {ALL_DATES.map(d => (
                      <div key={format(d, "yyyy-MM-dd")} className="flex-shrink-0 h-7 rounded-md bg-sidebar/50" style={{ width: `${COL_WIDTH}px` }} />
                    ))}
                  </div>
                  ...
                  <div className="flex gap-[3px] mt-[3px] mb-2">
                    <div
                      ...
                      style={{ width: `${nameColWidthForGrid}px`, position: "sticky", left: 0, zIndex: 20, background: "#1e293b" }}
                    >
                      On Duty
                    </div>
                    {ALL_DATES.map(d => {
                      const ds = format(d, "yyyy-MM-dd");
                      const isToday = ds === today;
                      const count = teamMembers.filter(m => ON_DUTY_CODES.has(shiftMap.get(m.id)?.get(ds)?.shiftCode ?? "")).length;
                      const numClass = isSurveillance
                        ? count > 5 ? "text-emerald-400" : count === 5 ? "text-yellow-400" : count >= 1 ? "text-red-400" : "text-slate-600"
                        : count >= 5 ? "text-emerald-400" : count >= 1 ? "text-slate-300" : "text-slate-600";
                      return (
                        <div key={ds} className={cn("flex-shrink-0 h-9 rounded-md flex items-center justify-center", isToday ? "bg-primary/20" : "bg-slate-800/70")} style={{ width: `${COL_WIDTH}px` }}>
                          <span className={cn("text-sm font-bold tabular-nums", numClass)}>{count}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex gap-[3px] mt-[3px] mb-2">
                    <div
                      className="flex-shrink-0 flex items-center px-3 h-9 rounded-md font-bold text-[10px] uppercase tracking-widest text-slate-400"
                      style={{ width: `${nameColWidthForGrid}px`, position: "sticky", left: 0, zIndex: 20, background: "#334155" }}
                    >
                      On Call
                    </div>
                    {ALL_DATES.map(d => {
                      const ds = format(d, "yyyy-MM-dd");
                      const isToday = ds === today;
                      const count = teamMembers.filter(m => ON_CALL_CODES.has(shiftMap.get(m.id)?.get(ds)?.shiftCode ?? "")).length;
                      const numClass = count >= 5 ? "text-emerald-400" : "text-red-400";
                      return (
                        <div key={ds} className={cn("flex-shrink-0 h-9 rounded-md flex items-center justify-center", isToday ? "bg-primary/20" : "bg-slate-700/60")} style={{ width: `${COL_WIDTH}px` }}>
                          <span className={cn("text-sm font-bold tabular-nums", numClass)}>{count}</span>
                        </div>
                      );
                    })}
          ...
          >
            ...
            <div style={{ minWidth: `${totalContentWidth}px`, width: `${totalContentWidth}px` }} className="relative pb-2">
              ...
              {(teamsData as Team[] ?? []).map(team => {
                const teamMembers = displayMembers.filter(m => m.teamId === team.id);
                const isDropTarget = overTeamId === team.id && activeDragId !== null;
                const isSurveillance = team.name === "Team 1" || team.name === "Team 2";
                  ...
                  <div key={team.id} className="mb-1">
                    ...
                    <div className="flex gap-[3px] mb-[3px]">
                      <div
                        className={cn(
                          ...
                          isDropTarget ? "bg-primary/20 text-primary" : "bg-sidebar text-sidebar-foreground",
                        )}
                        style={{
                          width: `${NAME_COL_WIDTH}px`,
                          position: "sticky",
                          left: 0,
                          zIndex: 20,
                          background: isDropTarget ? undefined : "var(--color-sidebar)",
                        }}
                      >
                        {team.name}
                      </div>
                      {ALL_DATES.map(d => (
                        <div
                          ...
                          className={cn("flex-shrink-0 h-7 rounded-md", isDropTarget ? "bg-primary/10" : "bg-sidebar/50")}
                    ...
                    <div className="flex gap-[3px] mt-[3px] mb-2">
                      ...
                      {ALL_DATES.map(d => {
                        ...
                        const count = teamMembers.filter(m =>
                          ON_DUTY_CODES.has(shiftMap.get(m.id)?.get(ds)?.shiftCode ?? "")
                        ).length;
                        const numClass = isSurveillance
                          ? count > 5 ? "text-emerald-400"
                            : count === 5 ? "text-yellow-400"
                            : count >= 1 ? "text-red-400"
                            : "text-slate-600"
                          : count >= 5 ? "text-emerald-400"
                          : count >= 1 ? "text-slate-300"
                          : "text-slate-600";
                        return (
                          <div
                            key={ds}
                    ...
                    <div className="flex gap-[3px] mt-[3px] mb-2">
                      ...
                      {ALL_DATES.map(d => {
                        ...
                        const isToday = ds === today;
                        const count = teamMembers.filter(m =>
                          ON_CALL_CODES.has(shiftMap.get(m.id)?.get(ds)?.shiftCode ?? "")
                        ).length;
                        const numClass = count >= 5 ? "text-emerald-400" : "text-red-400";
                        return (
                          <div
            ...
            <DragOverlay>
              {activeMember ? (
                <div className="flex items-center gap-2 px-3 h-11 bg-card border border-primary/40 rounded-md shadow-xl text-sm font-medium" style={{ width: `${NAME_COL_WIDTH}px` }}>
      ...
      {editSheet && (
        <ShiftEditSheet
          ...
          onSaveAnnotation={(comment, isActing) =>
            handleSaveAnnotation(editSheet.memberId, editSheet.date, editSheet.currentCode, comment, isActing)
          }
          onCopyShift={() => {
            const shift = shiftMap.get(editSheet.memberId)?.get(editSheet.date);
      ...
      <BulkCopySheet
        ...
        onCopy={(sourceMemberId, targetMemberIds, startDate, endDate) =>
          bulkCopy.mutate({ sourceMemberId, targetMemberIds, startDate, endDate })
        }
...
/* ── Team drop zone ───────────────────────────────────────────────────────── */
function TeamDropZoneRow({ teamId, totalWidth, isActive }: { teamId: number; totalWidth: number; isActive: boolean }) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isOver || isActive ? "border-primary/60 bg-primary/10" : "border-transparent",
      )}
    >
      {(isOver || isActive) && (
...
function MemberRow({
  member, allDates, today, shiftMap, isAdmin, bulkMode, bulkCopyMode, copiedBlock, selectedCells,
  nameColWidth, colWidth, rowHeight, clipboard, onCellTap, onCellPointerDown, onCellPointerEnter, onCopyCell, onPasteCell,
}: {
  member: Member; allDates: Date[]; today: string;
  shiftMap: Map<number, Map<string, ShiftData>>;
  isAdmin: boolean; bulkMode: boolean; bulkCopyMode: boolean;
  copiedBlock: { cells: { memberId: number; date: string; shiftCode: string; comment: string; isActing: boolean }[]; memberIds: number[]; dates: string[] } | null;
  selectedCells: Map<string, { memberId: number; date: string }>;
  nameColWidth: number; colWidth: number; rowHeight: number;
  clipboard: { code: string; comment: string; isActing: boolean } | null;
  onCellTap: (memberId: number, memberName: string, date: string, shift: ShiftData | undefined) => void;
  ...
  return (
    <div style={{ display: "flex", gap: "3px", marginBottom: "3px" }}>
      ...
      <div
        ...
        style={{ width: `${nameColWidth}px`, height: `${rowHeight}px`, position: "sticky", left: 0, zIndex: 10, background: "var(--color-card)" }}
      >
        <span className="flex-1 truncate text-sm font-medium text-foreground pl-3 pr-2">{member.name}</span>
      </div>
      ...
      {allDates.map(d => {
        ...
        const time = shift?.shiftTime ?? (SHIFT_TIMES[code as keyof typeof SHIFT_TIMES] ?? "");
        const isPasteMode = !!clipboard && isAdmin && !bulkMode && !bulkCopyMode && !copiedBlock;
        const isBlockPasteMode = !!copiedBlock && isAdmin;
        const isBulkCopySelected = bulkCopyMode && isSelected;
        return (
          <button
            key={ds}
            style={{ width: `${colWidth}px`, height: `${rowHeight}px`, flexShrink: 0, touchAction: "manipulation" }}
            className={cn(
              ...
              code
                ...
                : cn(isToday ? "bg-primary/10" : isWeekend ? "bg-muted/40" : "bg-muted/20", "hover:bg-muted/50"),
              isToday && code && "ring-2 ring-primary ring-offset-1",
              isActing && !isSelected && "ring-2 ring-amber-400 ring-offset-1",
              isSelected && !isBulkCopySelected && "ring-2 ring-primary ring-offset-1 brightness-90",
              isBulkCopySelected && "ring-2 ring-blue-400 ring-offset-1 brightness-90",
            )}
            onClick={() => { if (isPasteMode) { onPasteCell(member.id, ds); return; } onCellTap(member.id, member.name, ds, shift); }}
            onContextMenu={(e) => { if (isAdmin && !bulkMode && !bulkCopyMode) { e.preventDefault(); onCopyCell(shift); } }}
            onPointerDown={(e) => { e.stopPropagation(); onCellPointerDown(member.id, ds); }}
          ...
          >
            ...
            {hasComment && !isSelected && <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-white/80 shadow-sm" />}
            {isActing && !isSelected && <span className="absolute top-1 left-1"><Star className="w-2.5 h-2.5 text-amber-300 fill-amber-300 drop-shadow" /></span>}
            {isSelected && (
              <span className="absolute inset-0 flex items-center justify-center">
                <span className="w-5 h-5 rounded-full bg-primary/90 flex items-center justify-center shadow-sm"><CheckSquare2 className="w-3 h-3 text-white" /></span>
              </span>
            )}
            {!code && !isSelected && isAdmin && <span className="absolute inset-0 flex items-center justify-center text-muted-foreground/20 text-base font-light">+</span>}
          </button>
        );
      })}
    </div>
...
function SortableMemberRow({
  member, allDates, today, shiftMap, isAdmin, isDragging, bulkMode, bulkCopyMode, copiedBlock, selectedCells,
  nameColWidth, colWidth, rowHeight, clipboard, onCellTap, onCellPointerDown, onCellPointerEnter, onCopyCell, onPasteCell,
}: {
  member: Member; allDates: Date[]; today: string;
  shiftMap: Map<number, Map<string, ShiftData>>;
  isAdmin: boolean; isDragging: boolean; bulkMode: boolean; bulkCopyMode: boolean;
  copiedBlock: { cells: { memberId: number; date: string; shiftCode: string; comment: string; isActing: boolean }[]; memberIds: number[]; dates: string[] } | null;
  selectedCells: Map<string, { memberId: number; date: string }>;
  nameColWidth: number; colWidth: number; rowHeight: number;
  clipboard: { code: string; comment: string; isActing: boolean } | null;
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging: isSortableDragging } = useSortable({ id: member.id });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={cn("flex-1 truncate text-sm font-medium text-foreground pr-2", !isAdmin && "pl-3")}>
          {member.name}
        </span>
      </div>
        ...
        return (
          <button
            ...
            className={cn(
              ...
              code
                ? cn(shiftClass(code), "shadow-sm")
                : cn(
                    isToday ? "bg-primary/10" : isWeekend ? "bg-muted/40" : "bg-muted/20",
                    "hover:bg-muted/50"
                  ),
              isToday && code && "ring-2 ring-primary ring-offset-1",
              isActing && !isSelected && "ring-2 ring-amber-400 ring-offset-1",
            ...
            onClick={() => {
              if (isPasteMode) { onPasteCell(member.id, ds); return; }
          ...
          >
            ...
            {!code && !isSelected && isAdmin && (
              <span className="absolute inset-0 flex items-center justify-center text-muted-foreground/20 text-base font-light">+</span>
            )}
          </button>
        );
      })}
