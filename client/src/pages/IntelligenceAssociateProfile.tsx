...
function buildAssociateProfileHtml(profile: IntelAssociateProfile) {
...
.brand-label { font-size:10px; font-weight:600; letter-spacing:0.12em; text-transform:uppercase; color:${BLUE_MID}; margin-bottom:14px; }
.entity-name { font-size:22px; font-weight:700; } .gen-time { font-size:9px; opacity:0.6; margin-top:12px; }
.stats-row { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; padding:16px 32px; background:${BLUE_LIGHT}; border-bottom:2px solid ${BLUE_MID}; }
.stat-box { text-align:center; } .stat-num { font-size:20px; font-weight:700; color:${BLUE_DARK}; } .stat-label { font-size:9px; text-transform:uppercase; letter-spacing:0.08em; color:#64748b; }
.content { padding:20px 32px; } .section-title { font-size:11px; font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:${BLUE_DARK}; padding:6px 10px; background:${BLUE_LIGHT}; border-left:3px solid ${BLUE_MID}; margin-bottom:10px; }
...
<div class="content">
  ...
  ${profile.assocVehicles.length || profile.assocLocations.length ? `<div style="margin-bottom:16px"><div class="section-title">Associations</div>
    ${profile.assocVehicles.length ? `<div class="sub-title">Vehicles</div><div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:10px">${profile.assocVehicles.map(v => chipHtml(v.label, v.type, v.rowCount)).join("")}</div>` : ""}
...
export default function IntelligenceAssociateProfile() {
  ...
  function exportPdf() {
    if (!profile) return;
    const html = buildAssociateProfileHtml(profile);
