...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Local auth: session.openId stores the username
    const sessionUsername = session.openId;
    let user = await db.getUserByUsername(sessionUsername);

    // Fallback: try openId lookup for any legacy OAuth sessions
    if (!user) {
      user = await db.getUserByOpenId(sessionUsername);
    }
