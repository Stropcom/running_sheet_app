...
function clusterIntoCells(lines: Line[]): {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    for (let j = i + 1; j < segments.length; j++) {
      ...
      // The width-based guess above is a coin flip on a page where the
      // same x-bucket legitimately carries both narrow label cells and
      // much wider values (this column's own "colMaxWidth" then reflects
      // the wide ones, so a genuinely narrow forced mid-word break like
      // "PASSPO"/"RT" or "VEHICLE"/"S" never looks "packed" relative to
      // it) — checked only when the width guess said "not packed" (never
      // overrides a correct packed=true), and only forces a no-space
      // join when doing so produces a real word from the app's own known
