...
import {
  ...
  AttachmentUploadError as ExternalDocUploadError,
} from "./externalIntel/upload";
import {
...
export const appRouter = router({
  ...
  // Deterministic OCR + extraction pipeline (server/externalIntel/) — no
  // LLM/AI call anywhere in this path, per the Golden Rule in CLAUDE.md.
  // Nothing here is treated as final until an officer reviews/confirms it
  // (confirmReview) — same "no silent auto-attach" principle as the
  // existing entity-dedup flow above.
  externalIntel: router({
    /** Uploads a photographed/scanned document, runs OCR + field/entity extraction, and saves it pending review. */
    upload: protectedProcedure
      .input(
        z.object({
          operationId: z.number(),
      ...
      .mutation(async ({ input, ctx }) => {
        try {
          return await uploadExternalDocument({
            operationId: input.operationId,
            buffer: Buffer.from(input.dataBase64, "base64"),
            reportedMimeType: input.mimeType,
            fileName: input.fileName,
          ...
          if (err instanceof ExternalDocUploadError) {
            throw new TRPCError({
              code: err.status === 404 ? "NOT_FOUND" : "BAD_REQUEST",
              message: err.message,
            });
    ...
    list: protectedProcedure.query(async () => getExternalDocumentList()),

    get: protectedProcedure
      .input(z.object({ id: z.number() }))
      .query(async ({ input }) => getExternalDocumentById(input.id)),

    /** Officer corrections to the extracted demographic-table fields before confirming. */
    ...
    /** Fuzzy match suggestions for one extracted mention — reuses the same dedup engine as row-save/Merge Entities. */
    searchMatchesForMention: protectedProcedure
      .input(
        z.object({
          type: z.enum(["person", "vehicle", "address", "business"]),
          label: z.string().min(1),
    ...
    /** Officer confirms a mention as matched to an existing entity, a new entity, or not relevant. */
    resolveMention: protectedProcedure
      .input(
        z.object({
          mentionId: z.number(),
          status: z.enum(["matched", "new_entity", "rejected"]),
          matchedExistingKey: z.string().optional(),
        })
      )
      .mutation(async ({ input, ctx }) => {
        await resolveExternalEntityMention(
          input.mentionId,
          input.status,
          input.matchedExistingKey,
          ctx.user.cin ?? undefined
        );
        return { ok: true };
      }),

    /** Finalizes the document review — attaches the verbatim Summary to every resolved person/business mention. */
    confirmReview: protectedProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input, ctx }) => {
        await confirmExternalDocumentReview(
          input.id,
          ctx.user.cin ?? undefined
        );
        return { ok: true };
    ...
    delete: protectedProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input, ctx }) => {
        await softDeleteExternalDocument(input.id, ctx.user.cin ?? undefined);
        return { ok: true };
      }),
  }),
